...
The peak output figure is `resample`, the heaviest of the five stages at 66
bytes per sample. `process_cwa` allocates 50. If the machine starts swapping,
the timings are junk.
...
The three stages that allocate output differ by the arrays they ask the native
stage for. `resample` calls it with its defaults and gets all of them.
`load_cwa` skips the time array, which a `UniformRecording` computes from
`start_time`. `process_cwa` skips temperature as well, which a
`ProcessedRecording` does not carry.
